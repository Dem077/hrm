...
const filterTabs = computed<FilterTab[]>(() => {
    ...
    if (can('leave-requests.approve-hr')) {
        tabs.push({
            value: 'pending-hr',
...
function setFilter(filter: string) {
    ...
    router.get('/leave-requests', { filter }, { preserveState: true, replace: true });
}
</script>
...
<template>
    ...
    <AppLayout>
        <PageHeader
            ...
            description="Apply for leave, track your requests, and approve team leave when assigned as approver."
        >
            <template #actions>
                <UiButton v-if="canRecordForOthers" href="/leave-requests/record" variant="secondary">Record leave</UiButton>
                <UiButton v-if="canCreate && hasEmployeeProfile" href="/leave-requests/create" variant="primary">Apply for leave</UiButton>
            </template>
        </PageHeader>
        ...
        <EmptyState
            ...
            description="Leave applications you submit or need to approve will appear here."
        >
            <template v-if="canCreate && hasEmployeeProfile" #action>
                <UiButton href="/leave-requests/create" variant="primary">Apply for leave</UiButton>
            </template>
        </EmptyState>
                ...
                <table class="min-w-full text-sm">
                    <thead class="border-b border-slate-100 bg-slate-50/80 text-left text-slate-500 dark:border-slate-800 dark:bg-surface-elevated dark:text-slate-400">
                        <tr>
                            ...
                            <th class="px-5 py-3.5 font-medium">Dates</th>
                            <th class="px-5 py-3.5 font-medium">Days</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        <tr v-for="request in requests" :key="request.id" class="hover:bg-slate-50/60 dark:hover:bg-surface-elevated/60">
                            ...
                            <td class="px-5 py-4 text-slate-700 dark:text-slate-300">{{ request.leave_type?.name }}</td>
                            <td class="px-5 py-4 text-slate-600 dark:text-slate-400">
                                {{ formatDate(request.start_date) }} – {{ formatDate(request.end_date) }}
                            </td>
                            <td class="px-5 py-4 text-slate-700 dark:text-slate-300">{{ request.days_count }}</td>
                            ...
                            <td class="px-5 py-4 text-right">
                                <UiButton size="sm" :href="`/leave-requests/${request.id}`" variant="ghost">View</UiButton>
                            </td>
                        </tr>
                    </tbody>
                </table>
